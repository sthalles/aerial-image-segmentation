...
DenseNet-121 for image classification into 1000 classes:
   ...
   with slim.arg_scope(resnet_v2.densenet_arg_scope()):
...
def bottleneck(features, unit_id, growth_rate, depth_bottleneck, stride, theta, rate=1,
    ...
    with tf.variable_scope(scope, 'bottleneck', [features]) as sc:
        ...
        net = slim.conv2d(net, depth_bottleneck, [1, 1], scope="conv1", activation_fn=None, normalizer_fn=None)
        ...
        if unit_id > 0:
            net = tf.concat((net, features), axis=3, name="concat")
            ...
            with slim.arg_scope([slim.batch_norm], is_training=is_training):
                ...
                if include_root_block:
                    ...
                    with slim.arg_scope([slim.conv2d],
                        ...
                        net = densenet_utils.conv2d_same(net, 64, 7, stride=2, scope='conv1')
...
def densenet_121(inputs,
    ...
    blocks = [
        densenet_block('block1', growth_rate=32, num_units=6, stride=2, theta=0.5),
        densenet_block('block2', growth_rate=32, num_units=12, stride=2, theta=0.5),
        densenet_block('block3', growth_rate=32, num_units=24, stride=2, theta=0.5),
        densenet_block('block4', growth_rate=32, num_units=16, stride=1, theta=0.5),
    ]
