...
def transition_layer(inputs, factor, theta, scope=None):
    ...
    with tf.variable_scope(scope):
        if factor == 1:
            ...
            #net = slim.conv2d(net, theta * current_depth, [3, 3], stride=factor,
            #                  normalizer_fn=None, activation_fn=None, scope='downsampling')
            return net
            ...
            for i, unit in enumerate(block.args):
                ...
                with tf.variable_scope('unit_%d' % (i + 1), values=[net]):
                    ...
                    if output_stride is not None and current_stride == output_stride:
                        ...
                        net = block.unit_fn(net, i, rate=1, **dict(unit, stride=1))

            net = slim.utils.collect_named_outputs(outputs_collections, sc.name, net)

        with tf.variable_scope('transition_layer_%d' % (block_id + 1), [net]) as sc:
            if current_stride != output_stride:
                net=transition_layer(net, 2, 0.5, scope='transition_layer')
                current_stride*=2
...
def densenet_arg_scope(weight_decay=0.0001,
    ...
    TODO(gpapan): The batch-normalization related default values above are
      ...
      train ResNets from scratch, they might need to be tuned.
    ...
    Returns:
      An `arg_scope` to use for the deeplab models.
    """
    batch_norm_params = {
